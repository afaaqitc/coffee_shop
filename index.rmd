...
setwd("~/Data/Learn/R_Projects/coffee/")

fi<-read.csv(file = "nn .csv")
...
month_totals_chart<-
  ...
  geom_text(aes(label=total),size=3.5,fontface = "bold",color="white",position = position_stack(vjust = 0.5))+
  theme_bw()+
...
geom_bar(stat = "identity")+
  geom_text(aes(label=total),size=2.5,color="white",
...
geom_text(aes(label=total),size=4.5,color="white",
position = position_stack(vjust = 0.5))+
scale_fill_brewer(palette = "Set1")+
  labs(x="",y="",
       subtitle = paste0("Total: ",comma(sum(fi$money),2)," | ",
...
geom_text(aes(label=profit),size=4.5,color="white",
position = position_stack(vjust = 0.5))+
scale_fill_brewer(palette = "Set1")+
labs(x="",y="",
subtitle = paste0("Total: ",comma(sum(fi$money),2)," | ",
...
### Sales
```{r}
valueBox(value = comma(sum(fi$money)),caption ="Sales" ,icon ="fa fa-money" ,color ="#b2bec3" )
```
